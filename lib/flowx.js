...
export default class Flowx {
  ...
  */
  ...
  async endDragOldNode() {
    ...
    let newBlocks = this.blocks.map((b) => ({ ...b }))
    let copiedBlocks = []
    ...
    if (parent != null) {
      // 当移动元素时，我们总是复制一份新元素，然后替换原来的元素
      // 因为 d3.js 布局算法中，同层会按照 id 大小顺序排列，这样可以保证拖动的元素永远在原有元素的后面
      ...
      if (!this.isCopy) {
        newBlocks = newBlocks.filter(
          (blc) => !this.draggingBlocks.find((dblc) => dblc.id === blc.id)
        )
        this.draggingBlocks.forEach((blc) => blc.el.remove())
      }
