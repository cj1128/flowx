...
export default class Flowx {
  ...
  async import(tree) {
    ...
    await Promise.all(
      this.blocks.map(async (b) => {
        b.el = await this.renderBlock(b.data, b.id)
        return b
      })
    )
  ...
  */
  ...
  removeNode(id) {
    ...
    const parent = this.findParentInTree(tree, id)
    parent.children = parent.children.filter((c) => c.id !== id)

    this.import(tree)
  }

  findParentInTree(tree, id) {
    if (tree.children.find((i) => i.id === id)) return tree

    for (let i = 0; i < tree.children.length; i++) {
      const node = this.findParentInTree(tree.children[i], id)
      if (node) return node
    }
  ...
  async renderBlock(data, id) {
    ...
    await this.config.onRender(data, $app, {
      delete: () => this.removeNode(id),
    })
    return $container
  }
}
