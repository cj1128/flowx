...
export default class Flowx {
  constructor(cfg = {}) {
    this.config = {
      ...
      onResizeNode: cfg.onResizeNode || emptyFn,
  ...
  resize({ nodeHeight, nodeWidth, marginX, marginY }) {
    this.config.nodeHeight = nodeHeight
    this.config.nodeWidth = nodeWidth
    this.config.marginX = marginX
    this.config.marginY = marginY
    this.blocks.forEach((b) => {
      b.el.style.width = `${nodeWidth}px`
      b.el.style.height = `${nodeHeight}px`
      this.config.onResizeNode(b.data)
    })
    this.layout()
  }
