...
export default class Flowx {
  ...
  zoom(x) {
    ...
    this.blocks.forEach((b) => {
      b.el.style.width = `${this.getRenderSize().nodeWidth}px`
      b.el.style.height = `${this.getRenderSize().nodeHeight}px`
      b.el.childNodes[0].style.zoom = x
  ...
  */
  ...
  beginDragNewNode(evt) {
    const original = evt.target.closest(".flowx-drag")
  ...
  layout() {
    ...
    this.d3Tree = tree().nodeSize([
      this.getRenderSize().nodeHeight + this.getRenderSize().marginY,
      this.getRenderSize().nodeWidth + this.getRenderSize().marginX,
    ])(treeData)
  ...
  drawLinks() {
    ...
    this.svg
      ...
      .attr("d", (d) => {
        return (
          "M" +
          (d.source.data.left + this.getRenderSize().nodeWidth) +
          "," +
          (d.source.data.top + this.getRenderSize().nodeHeight / 2) +
          "h" +
          this.getRenderSize().marginX / 2 +
          "v" +
  ...
  async renderBlock(block) {
    ...
    $container.style.width = this.getRenderSize().nodeWidth + "px"
    $container.style.height = this.getRenderSize().nodeHeight + "px"
    ...
    $container.childNodes[0].style.zoom = this._zoom
    return $container
  }
}
