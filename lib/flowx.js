...
export default class Flowx {
  constructor(cfg = {}) {
    this.config = {
      ...
      onPrefixRender: cfg.onPrefixRender,
      ...
      marginX: cfg.marginX || 50, // 节点之间横向距离
      marginY: cfg.marginY || 20, // 节点之间纵向距离
      outlineColor: cfg.outlineColor || '#217ce8',
    }
    this.blocks = []
  ...
  export() {
    ...
    const rootBlock = this.blocks.find(b => b.parentId === '')
    const tree = this.blocks2Tree(this.blocks)

    return { tree, left: rootBlock.left, top: rootBlock.top }
  }
  ...
  */
  blocks2Tree(blocks) {
    // sortKey 从小到大排，这样生成的 tree 的 children 中 sortKey 大的会在后面
    const sorted = blocks.sort((a, b) => a.sortKey - b.sortKey)
  ...
  _tree2Blocks(tree, parentId, blocks) {
    const block = this.newBlock({ id: tree.id, data: tree.data, parentId })
    blocks.push(block)
    tree.children.forEach(t => this._tree2Blocks(t, block.id, blocks))
  }

  newBlock({ id, data, parentId = '' }) {
    return {
      id,
      data,
      // 用于 D3 布局时对节点进行排序
  ...
  beginDragNewNode(evt) {
    ...
    this.dragEl = newNode
    this.dragElData = this.getFlowxData(origin)
    this.dragX = dragX
    this.dragY = dragY
  }
  ...
  onNodeMove(event) {
    ...
    this.blocks.forEach(b => (b.el.style.outline = ''))
    ...
    if (target) {
      target.el.style.outline = `2px solid ${this.config.outlineColor}`
    }
  }
  ...
  async endDragOldNode() {
    ...
    const child = newBlocks.find(b => b.id === this.dragAnchor.dataset.flowxId)
    ...
    if (parent != null) {
      if (this.isCopy) {
        ...
        child.sortKey = Date.now() // 保证当前 node 排在下面
  ...
  async endDragNewNode() {
    ...
    const block = this.newBlock({ id: genID(), data: this.dragElData })
  ...
  layout() {
    ...
    const treeData = stratify()
      .id(n => n.id)
      .parentId(n => n.parentId)(this.blocks)
      // 按照 sortKey 从小到大排，sortKey 大的会在下方
  ...
  async renderBlock(block) {
    ...
    const props = {
      data: block.data,
      id: block.id,
      isRoot: block.parentId === '',
    }
    await this.config.onRender(props, $app)

    // render prefix
    if (this.config.onPrefixRender != null) {
      const $prefixContainer = document.createElement('div')
      $prefixContainer.classList.add(`flowx-prefix`)
      $prefixContainer.style.left = -this.config.marginX / 4 + 'px'
      $container.appendChild($prefixContainer)
      const $prefix = document.createElement('div')
      $prefixContainer.appendChild($prefix)
      await this.config.onPrefixRender(props, $prefix)
    }
