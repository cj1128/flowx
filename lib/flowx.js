...
export default class Flowx {
  ...
  beginDragOldNode(evt) {
    ...
    const targetId = Number(target.dataset.flowxId)

    const newNode = this.canvasElement.cloneNode(true)
    this.dragX = evt.clientX - newNode.getBoundingClientRect().left
    this.dragY = evt.clientY - newNode.getBoundingClientRect().top
    ...
    this.blocks.forEach((b) => {
      if (this.draggingBlocks.find((item) => item.id === b.id)) return

      const clonedBlock = newNode.querySelector(`[data-flowx-id="${b.id}"]`)
      clonedBlock.parentNode.removeChild(clonedBlock)

      const clonedPath =
        newNode.querySelector(`[data-flowx-target="${b.id}"]`) ||
        newNode.querySelector(`[data-flowx-source="${b.id}"]`)

      clonedPath && clonedPath.remove()
    })

    this.canvasElement.appendChild(newNode)
    // 移除子树根节点与父级的连线
    const clonedParent = newNode.querySelector(
      `[data-flowx-target="${targetId}"]`,
    )
    clonedParent && clonedParent.remove()
  ...
  onNodeMove(event) {
    ...
    const target = this.blocks.find((b) =>
      this.checkAttach(b.el, this.dragAnchor || this.dragEl),
    )
    if (target) {
  ...
  endDragOldNode() {
    ...
    const parent = this.blocks.find((blc) =>
      this.checkAttach(blc.el, this.dragAnchor),
    )

    const clear = () => {
      parent && (parent.el.style.border = "")
      this.dragAnchor = null
      this.draggingBlocks = null
    ...
    const subRootId = Number(this.dragAnchor.dataset.flowxId)
    const subRootBlock = this.blocks.find((b) => b.id === subRootId)

    // 父节点不能是子树节点
    if (!this.draggingBlocks.find((b) => b.id === parent.id)) {
      subRootBlock.parent = parent.id
      this.layout()
    }
