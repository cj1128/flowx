...
export default class Flowx {
  constructor(cfg = {}) {
    ...
    this.originSize = {
      ...
      nodeHeight: this.config.nodeHeight,
  ...
  zoom(x) {
    ...
    this.config.nodeHeight = this.originSize.nodeHeight * x
    this.config.nodeWidth = this.originSize.nodeWidth * x
    this.config.marginX = this.originSize.marginX * x
    this.config.marginY = this.originSize.marginY * x
    this._zoom = x

    this.blocks.forEach((b) => {
      b.el.style.width = `${this.config.nodeWidth}px`
      b.el.style.height = `${this.config.nodeHeight}px`
      b.el.childNodes[0].style.zoom = x
      this.config.onZoom(b.data)
      this.config.onUpdate(b.data)
    })
